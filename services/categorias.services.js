...
module.exports.articulosCategoria = async (categoria) => {
    try {
        let resultado = await sequelize.query(`SELECT * FROM articulos WHERE cat = '${categoria.id}'`)
        return resultado
    } catch (error) {
        console.log(error)
        throw new Error ('Ocurrio error al obtener los articulos de la categoria.')
    }
}